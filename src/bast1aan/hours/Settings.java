...
public class Settings {
	...
	public Settings() {
		...
		if (settingsUrl == null) {
			throw new HoursException("settings.properties not found");
		}
			...
			for (Field field: fields) {
				if (Modifier.isStatic(field.getModifiers()) && field.getType() == String.class) {
					...
					try {
						...
						throw new HoursException("Error reading field from Settings class", e);
					}
					settings.put(key, prop.getProperty(key));
				}
			}
		} catch (IOException e) {
			throw new HoursException("error reading settings.properties", e);
		} finally {
			if (stream != null) {
				try {
					stream.close();
				} catch (IOException e) {
					throw new HoursException("error closing settings.properties", e);
				}
			}
		}
